...
import torch.nn as nn  ### (existing) needed for L1Loss
...
class Pix2PixHDModel(BaseModel):
    ...
    ########################################################################
    ### Updated to handle no_l1_loss plus the L1 loss weighting
    ########################################################################
    def init_loss_filter(self, use_gan_feat_loss, use_vgg_loss, use_l1_loss):  
        ### We'll add a new slot for L1:
    ...
    def initialize(self, opt):
        ...
        if opt.resize_or_crop != 'none' or not opt.isTrain:
            # when training at full resolution, this can help
        ...
        if self.isTrain:
            ...
            self.netD = networks.define_D(
                netD_input_nc, opt.ndf, opt.n_layers_D, opt.norm, use_sigmoid,
                opt.num_D, not opt.no_ganFeat_loss, gpu_ids=self.gpu_ids,
                use_full_image=opt.use_full_imageD  # from user param
            )

        # Encoder network (for instance-wise features)
        if self.gen_features:
            self.netE = networks.define_G(
                opt.output_nc, opt.feat_num, opt.nef, 'encoder',
        ...
        if self.isTrain:
            ...
            ### We add "use_l1_loss" (the user sets no_l1_loss to True/False):
            self.loss_filter = self.init_loss_filter(
                not opt.no_ganFeat_loss,
                not opt.no_vgg_loss,
                not opt.no_l1_loss,  ### Added for L1 Loss
            )
            ...
            # Names so we can break out loss
            # Now we have an extra slot for L1:
            self.loss_names = self.loss_filter(
                'G_GAN', 'G_GAN_Feat', 'G_VGG', 'D_real', 'D_fake', 'G_L1'
            )
            ...
            if opt.niter_fix_global > 0:
                ...
                print(('------------- Only training the local enhancer network '
                       '(for %d epochs) ------------' % opt.niter_fix_global))
    ...
    def forward(self, label, inst, image, feat, infer=False):
        ...
        return [
            self.loss_filter(
                ...
                loss_D_real, loss_D_fake, loss_G_L1
            ),
    ...
    def sample_features(self, inst):
        ...
        for i in np.unique(inst_np):
            ...
            if label in features_clustered:
                ...
                feat = feat[feat[:,-1] > 0.5, :-1]    # optional filtering
                if feat.shape[0]:
                    n_clusters = min(feat.shape[0], self.opt.n_clusters)
                    # pick one cluster center or random row
                    idx = np.random.randint(0, feat.shape[0])
                    cluster_center = feat[idx]
                    idx_pix = (inst == int(i)).nonzero()
                    for k in range(self.opt.feat_num):
                        feat_map[idx_pix[:,0], idx_pix[:,1] + k, idx_pix[:,2], idx_pix[:,3]] = cluster_center[k]
        if self.opt.data_type==16:
