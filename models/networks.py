...
def define_D(input_nc, ndf, n_layers_D, norm='instance', use_sigmoid=False,
             num_D=1, getIntermFeat=False, gpu_ids=[], use_full_image=False):
    """
    :param use_full_image: if True, produce a single scalar real/fake output for the entire image at each scale.
                           if False, the original patch-based output is used (like PatchGAN).
    """
    norm_layer = get_norm_layer(norm_type=norm)
    netD = MultiscaleDiscriminator(
        input_nc, ndf, n_layers_D, norm_layer, use_sigmoid,
        num_D, getIntermFeat, use_full_image
    )
    print(netD)
    if len(gpu_ids) > 0:
        assert(torch.cuda.is_available())
...
class LocalEnhancer(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=32, n_downsample_global=3,
        ...
        model_global = GlobalGenerator(
            input_nc, output_nc, ngf_global, n_downsample_global,
        ...
        for n in range(1, n_local_enhancers+1):
            ...
            # upsample
            model_upsample += [
                nn.ConvTranspose2d(ngf_global*2, ngf_global, kernel_size=3, stride=2,
...
class MultiscaleDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d,
                 use_sigmoid=False, num_D=3, getIntermFeat=False, use_full_image=False):
        """
        :param use_full_image: If True, each scale gives a single scalar output
                               for real/fake. Otherwise, patch-based grid output.
        """
    ...
    def forward(self, input):
        ...
        for i in range(self.num_D):
            if self.getIntermFeat:
                model = [
                    getattr(self, 'scale' + str(self.num_D - 1 - i) + '_layer' + str(j))
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d,
                 use_sigmoid=False, getIntermFeat=False, use_full_image=False):
        """
        :param use_full_image: if True, the last layer outputs a single scalar per image
                               rather than a grid (patch-based).
        """
        ...
        if not use_full_image:
            # Original patch-based final layer
            sequence += [[nn.Conv2d(nf, 1, kernel_size=kw, stride=1, padding=padw)]]
            if use_sigmoid:
                sequence += [[nn.Sigmoid()]]
        else:
            # Single-scalar final layer
            final_seq = []
            # global average pooling -> 1×1 conv -> optional Sigmoid
            final_seq += [nn.AdaptiveAvgPool2d(1)]
            final_seq += [nn.Conv2d(nf, 1, kernel_size=1, stride=1, padding=0)]
            if use_sigmoid:
                final_seq += [nn.Sigmoid()]
            sequence += [final_seq]
